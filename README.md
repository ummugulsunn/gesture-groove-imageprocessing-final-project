...
An interactive application that plays music in real-time by detecting hand gestures.

## 🎯 Project Overview

GestureGroove uses computer vision and machine learning techniques to:
- **Detect hand gestures in real-time** (MediaPipe-based)
- **Classify different gesture types** (open hand, fist, pointing finger, etc.)
- **Play different musical sounds for each gesture** (Piano, Drums, Synth)
- **Offer Beat Challenge mode** for rhythm games
- **Support multiple instruments** and sound effects
...
```bash
# REQUIRED: Python 3.11 (for MediaPipe support)
...
- **4GB+ RAM**: For MediaPipe and TensorFlow Lite
...
4. **Listen to music**: Each gesture produces different sounds
...
| ✌️ **Peace** | Index and middle fingers open | G4 note | Ride cymbal |
...
gesturegroove/
├── src/
│   ├── gesture_recognition/    # Hand detection
│   ├── audio_engine/          # Sound system
│   ├── ui/                    # User interface
